...
### mediapipe_receiver_utility (デバッグ用ツール)

改造版mediapipeのデータの表示・保存・再生を行うツールです。
使用方法はmediapipe_receiver_utility_vYYYYMMDD
(同梱されているバージョンに読み替えてください)
のREADME.pdfを参照してください。
...
(11)のプルダウンメニューはレベルを選択することができます。
GreenBack(グリーンバック),
FountainPark(公園)から選択できます。

レベルにGreenBackを指定している場合は
(12)背景色の変更ボタンを押すと、各色に背景を変更することができます。
緑色が使われているモデルなど、
グリーンバックで色が抜けない場合にご利用ください。

(13)鏡像反転のチェックボックスを有効にすると
姿勢をすべて左右方向に反転します。
スマホのインカメラは鏡像となっているため、チェックすることで
本来のご自身の動きを反映したものになります。

(14)下半身トラッキングのチェックボックスを外すと、
